...
build_sub_stx_06 <- function() {
  ...
  est_rem <- subdistrict_estates() %>% filter(subdist %in% c(5,8, 9)) %>% pull(subdist_estate) %>% sort()
  ...
  rms <- c("Clifton", "VI C.*[BCD].$", "Frederiksted", "Two Brothers")
  inc <- c()
